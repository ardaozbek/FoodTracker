...
def index():
    ...
    if request.method == 'POST':
        ...
        mdt = dt.strptime(date, '%Y-%m-%d')
        database_date = dt.strftime(mdt, '%Y%m%d')
    ...
    for i in results:
        ...
        d = dt.strptime(str(i['entry_date']), '%Y%m%d')
...
def view(date):
    ...
    log_cur = db.execute('SELECT food.name, food.protein, food.carbohydrates, food.fat, food.calories FROM log_date JOIN food_date ON log_date.id = food_date.log_date_id \
                JOIN food ON food_date.food_id = food.id WHERE log_date.entry_date = ?', [date])
